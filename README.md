...
| 🤖 LangGraph ReAct Agent | 推理 → 工具调用 → 观察 → 再推理，`recursion_limit=12` 防止无限循环，`MemorySaver` 进程内检查点 |
...
├── README.md                       # 本文件
├── doc/                            # 设计与对齐文档
│   ├── API.md                      # 权威 API 文档 (355 行)
│   └── API对齐.md                  # 认证契约对齐说明
├── Frontend/                       # 前端目录（待开发，React/Vite）
└── backend/                        # 后端服务
...
- [x] 训练任务全生命周期管理
- [x] 双通道 JWT 认证
- [ ] 对话历史断点继续 + 创建新对话（自定义 DB 持久化）
- [ ] 前端开发（React / Vite，端口 5173）
